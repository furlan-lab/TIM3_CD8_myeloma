...
RidgePlot(seu, assay = "HTO", features = rownames(seu[["HTO"]])[1:3])

DefaultAssay(seu)<-"HTO"
VlnPlot(seu, features =rownames(seu), pt.size = 0)

#adding log normalized counts of HTOS to metadata
seu$log_ht1<-seu@assays$HTO@data["HT1",] %>% log10()
seu$log_ht2<-seu@assays$HTO@data["HT2",] %>% log10()
seu$log_ht3<-seu@assays$HTO@data["HT3",] %>% log10()

#export seurat metadata
meta_df<-seu@meta.data

names(meta_df)

#plot hto1 count distribution, you you can change orig ident to dataset
#the y intercept will be your chosen threshold
ggplot(meta_df, aes(x = orig.ident, y = log_ht1))+geom_violin()+geom_hline(yintercept = 0)
ggplot(meta_df, aes(x = orig.ident, y = log_ht2))+geom_violin()+geom_hline(yintercept = 0)
ggplot(meta_df, aes(x = orig.ident, y = log_ht3))+geom_violin()+geom_hline(yintercept = 0)

#seems like log normalizing it makes it nicely centered at 0, you might have different values
thresh<-c(0,0,0)

#make new metadata for experimental condition, im calling it id, it can be whatever
seu$id<-seu$dataset

#in this experiment the pools/samples were called ip or BM, you want to change this to each sample name
#this is where you need to read the excel spreadsheet and figure out which timepoint/sample is in 
#which pool and is postiive for which HTO (aka greater than threshold)
seu$id[seu$dataset == "IP" & seu$HTO1 > thresh[1]]<- "patient742_ip"
seu$id[seu$dataset == "IP" & seu$HTO2 > thresh[2]]<- "patient871_ip"
seu$id[seu$dataset == "BM" & seu$HTO3 > thresh[3]]<- "patient742_bm"



ht1_pos<-colnames(seu)[which(seu@assays$HTO@data["HT1",] > 2)]

colnames(seu)[which(seu@assays$HTO@data["HT2",] > 2)]

colnames(seu)[which(seu@assays$HTO@data["HT3",] > 2)]
```
